...
| Actually empty node never removed | Check NodePool budgets for `Empty` disruptions and the consolidation policy, then inspect Karpenter's decision; a pod PDB cannot block eviction from a node with no pods. [Karpenter disruption](https://karpenter.sh/docs/concepts/disruption/) |
